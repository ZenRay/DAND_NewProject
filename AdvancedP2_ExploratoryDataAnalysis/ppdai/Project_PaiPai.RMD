...
# 加载数据
df.lc <- read.csv("./LC.csv", na.strings = c("NA", "NULL"))
df.lp <- read.csv("./LP.csv", na.strings = c("NA", "NULL"))
df.lcis <- read.csv("./LCIS.csv", na.strings = c("NA", "NULL"))
```
...
apply(is.na.data.frame(df.loan), 2, sum)

toString("The amount of missing value about investment:")
apply(is.na.data.frame(df.lcis), 2, sum)
```

```{r include=FALSE}
# explore the loan missing value
nrow(filter(df.loan, 还款状态=="未还款" & is.na(df.loan$还款日期))) == sum(is.na(df.loan$还款日期 ))
```

从以上的结果来看，Loan 数据中存在缺失数据的字段为 **还款日期**。从对还款日期和还款状态来看，还款日期缺失是因为该债务状态是没有开始还款；在 Invenstment 数据中存在缺失数据的字段为 **上次还款日期** 和 **recorddate**。 **recorddate** 的缺失值中和 **上次还款日期** 的缺失值也是同期的，但还需要对 **上次还款日期** 进行分析。
